...
class Decoder(nn.Module):
    def __init__(self, args, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False, up_type=0):
        ...
        if "LA" in args.dataset:
            attention_size = [7, 7, 5]
        else:
            attention_size = [int(size) // 16 for size in args.patch_size]
            if any(size <= 0 for size in attention_size):
                raise ValueError("patch_size must be at least 16 in every dimension")
        self.attn_x5 = CA_TransformerDecoderLayer(
            input_size=attention_size, in_channels=256, d_model=512,
            patch_size=attention_size
        )
